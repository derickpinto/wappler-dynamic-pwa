(this.workbox = this.workbox || {}),
  (this.workbox.backgroundSync = (function (t, e, s, i, n) {
    ...
    class c {
      constructor(t) {
        (this.t = t),
          (this.i = new n.DBWrapper("workbox-background-sync", 3, {
            onupgradeneeded: this.h,
          }));
      }
      async pushEntry(t) {
        delete t.id, (t.queueName = this.t), await this.i.add(a, t);
      }
      async unshiftEntry(t) {
        const [e] = await this.i.getAllMatching(a, { count: 1 });
        e ? (t.id = e.id - 1) : delete t.id,
          (t.queueName = this.t),
          await this.i.add(a, t);
      }
      async popEntry() {
        return this.o({ direction: "prev" });
      }
      async shiftEntry() {
        return this.o({ direction: "next" });
      }
      async getAll() {
        return await this.i.getAllMatching(a, {
          index: r,
          query: IDBKeyRange.only(this.t),
        });
      }
      async deleteEntry(t) {
        await this.i.delete(a, t);
      }
      async o({ direction: t }) {
        const [e] = await this.i.getAllMatching(a, {
          direction: t,
          index: r,
          query: IDBKeyRange.only(this.t),
          count: 1,
    ...
    class f {
      constructor(t, { onSync: s, maxRetentionTime: i } = {}) {
        if (((this.l = !1), (this.q = !1), y.has(t)))
          throw new e.WorkboxError("duplicate-queue-name", { name: t });
        y.add(t),
          (this.m = t),
          (this.p = s || this.replayRequests),
          (this.g = i || 10080),
          (this.R = new c(this.m)),
          this.k();
      }
      ...
      async getAll() {
        const t = await this.R.getAll(),
          e = Date.now(),
          s = [];
        for (const i of t) {
          const t = 60 * this.g * 1e3;
          e - i.timestamp > t ? await this.R.deleteEntry(i.id) : s.push(w(i));
        }
        return s;
      }
      async D({ request: t, metadata: e, timestamp: s = Date.now() }, i) {
        const n = {
          requestData: (await o.fromRequest(t.clone())).toObject(),
          timestamp: s,
        };
        e && (n.metadata = e),
          await this.R[i + "Entry"](n),
          this.l ? (this.q = !0) : await this.registerSync();
      }
      async _(t) {
        const e = Date.now(),
          s = await this.R[t + "Entry"]();
        if (s) {
          const i = 60 * this.g * 1e3;
          return e - s.timestamp > i ? this._(t) : w(s);
        }
      }
        ...
        for (; (t = await this.shiftRequest()); )
          try {
            ...
            throw (
              ...
              new e.WorkboxError("queue-replay-failed", { name: this.m }))
            );
          }
      }
      async registerSync() {
        if ("sync" in self.registration)
          try {
            await self.registration.sync.register(`${u}:${this.m}`);
          } catch (t) {}
      }
      k() {
      ...
      (t.BackgroundSyncPlugin = class {
        constructor(t, e) {
          (this.fetchDidFail = async ({ request: t }) => {
            await this.S.pushRequest({ request: t });
          }),
            (this.S = new f(t, e));
        }
      }),
